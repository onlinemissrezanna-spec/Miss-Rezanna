...
// Replace CSS link and JS script tag with inline styles and inline scripts
const inlinedHtml = html
  .replace(/<link rel="stylesheet" href="css\/admin\.css[^"]*">/, `<style>${css}</style>`)
  .replace(/<script src="js\/admin\.js[^"]*"><\/script>/, `<script>${js}</script>`);
